...
class QLearningAgent(ReinforcementAgent):
    ...
    def getQValue(self, state, action):
        ...
        "return the value for the state action"
    ...
    def computeValueFromQValues(self, state):
        ...
        "variable to find best action"
        highestQ = float('-inf')
        "loop through al find highest q value"
        for action in legalActions:
          if self.getQValue(state, action) > highestQ:
            highestQ = self.getQValue(state, action)
        return highestQ
    ...
    def computeActionFromQValues(self, state):
        ...
        "get legal actions"
        legalActions = self.getLegalActions(state)
        "default best is none"
        bestAction = None
        "variable to find highest q value"
        maxQValue = float ('-inf')
        ...
        "for loop through all actions and find highest q value"
        for action in legalActions:
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        "difference equation from slides"
        difference = reward + self.discount * self.getValue(nextState) - self.getQValue(state, action)

        "loop through all features in the vector, and update their weights"
